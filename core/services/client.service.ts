...
class ClientService {
  ...
  async edit(clientId: string, body: ClientEditBody): Promise<Client> {
    await axios.put<Client>(`${this.BASE_URL}/${clientId}/name`, {
      name: body.name
    }, {
      headers: this.getAuthHeaders()
    })
    const { data } = await axios.put<Client>(`${this.BASE_URL}/${clientId}/address`, {
  ...
  async configuration(clientId: string): Promise<string> {
    const response = await axios.get<BlobPart>(`${this.BASE_URL}/${clientId}/configuration`, {
      headers: this.getAuthHeaders(),
      responseType: 'blob'
    })
  ...
  async qrcode(clientId: string): Promise<string> {
    const { data } = await axios.get<QRCodeResponse>(`${this.BASE_URL}/${clientId}/qrcode`, {
      headers: this.getAuthHeaders(),
    })
